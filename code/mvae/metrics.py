...
class Metrics():
    def __init__(self, data, pred, K_vec):
        ...
        self.text_mse = MSE(data.text.detach().cpu().numpy(), pred.text)
        ...
        self.bp_mse = MSE(data.bp.cpu().numpy(), pred.bp)
        self.indus_mse = MSE(data.indus.detach().cpu().numpy(), pred.indus)
        
        self.text_mad = mad(data.text.detach().cpu().numpy(), pred.text)
        ...
        self.bp_mad = mad(data.bp.cpu().numpy(), pred.bp)
        self.indus_mad = mad(data.indus.detach().cpu().numpy(), pred.indus)
        
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            self.text_report = classification_report(data.text.detach().cpu().numpy(), pred.text > 0.5, output_dict = True)
            ...
            self.indus_report = classification_report(data.indus.detach().cpu().numpy(), pred.indus > 0.5, output_dict = True)
            
            fpr, tpr, thresholds = roc_curve(data.text.cpu().numpy().flatten(), pred.text.flatten())
            self.text_auc = auc(fpr, tpr)
            
            fpr, tpr, thresholds = roc_curve(data.bin.cpu().numpy().flatten(), pred.bin.flatten())
            self.bin_auc = auc(fpr, tpr)
            
            fpr, tpr, thresholds = roc_curve(data.indus.cpu().numpy().flatten(), pred.indus.flatten())
            self.indus_auc = auc(fpr, tpr)          
