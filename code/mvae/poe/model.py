...
class LogoMVAE(nn.Module):
    ...
    def __init__(self, z_dim, encoder_dims, decoder_dims, x_sizes, use_cuda=True):
        super().__init__()
        self.encoder = PoEEncoder(z_dim, encoder_dims, x_sizes)
    ...
    # define the model p(x|z)p(z1)
    def model(self, data, domain_mask, annealing_factor=1.):
    ...
    def guide(self, data, domain_mask, annealing_factor=1.):
        ...
        logo_inputs = torch.cat((data.bin, data.cat1_hot, data.cat2_hot, data.cat3_hot, data.cat4_hot, data.cat5_hot), 1)
        bp_inputs = data.bp
        indus_inputs = data.indus

        with pyro.plate("data", N):                
            z_loc, z_scale = self.encoder.forward(text_inputs, logo_inputs, bp_inputs, indus_inputs, domain_mask)
            with poutine.scale(None, annealing_factor):
                z = pyro.sample("latent", dist.Normal(z_loc, z_scale).to_event(1))
    
            
    def est_z(self, test_data, domain_mask):
        self.z = NewZ(self, test_data, domain_mask)
        
    def pred_x(self, test_data, domain_mask, samples=100):
        self.x_pred = Predict(self, NewZ(self, test_data, domain_mask), samples=100)
    
    def predict(self, test_data, domain_mask, samples=100):
        self.pred = LMVAE_Test(self, test_data, domain_mask, self.K_vec, samples)

        
class PoEEncoder(nn.Module):
    def __init__(self, z_dim, encoder_dims, x_sizes):
        super().__init__()
        # set up the three linear transformations used
        self.text_inf = Encoder(z_dim, encoder_dims["text"], x_sizes["text"])
        self.logo_inf = Encoder(z_dim, encoder_dims["logo"], x_sizes["logo"])
        self.bp_inf = Encoder(z_dim, encoder_dims["bp"], x_sizes["bp"])
        self.indus_inf = Encoder(z_dim, encoder_dims["indus"], x_sizes["indus"])
        
        self.z_dim = z_dim
        
    def forward(self, x_text, x_logo, x_bp, x_indus, domain_mask):
        z_loc_text, z_scale_text = self.text_inf(x_text)
        z_loc_logo, z_scale_logo = self.logo_inf(x_logo)
        z_loc_bp, z_scale_bp = self.bp_inf(x_bp)
        z_loc_indus, z_scale_indus = self.indus_inf(x_indus)
        
        z_scale = 1./(domain_mask[:,0,None]*(1./z_scale_text) + domain_mask[:,1,None]*(1./z_scale_logo) + domain_mask[:,2,None]*(1./z_scale_bp) + domain_mask[:,3,None]*(1./z_scale_indus) + torch.ones(self.z_dim).cuda())
        prec_wt_loc = domain_mask[:,0,None]*(z_loc_text / z_scale_text) + domain_mask[:,1,None]*(z_loc_logo / z_scale_logo) + domain_mask[:,2,None]*(z_loc_bp / z_scale_bp) + domain_mask[:,3,None]*(z_loc_indus / z_scale_indus)
        z_loc = prec_wt_loc * z_scale
...
class NewZ():
    def __init__(self, lmvae, data, domain_mask, samples=100):
        with torch.no_grad():
            logo_inputs = torch.cat((data.bin, data.cat1_hot, data.cat2_hot, data.cat3_hot, data.cat4_hot, data.cat5_hot), 1)
            self.z_loc, self.z_scale = lmvae.encoder.forward(data.text, logo_inputs, data.bp, data.indus, domain_mask)
...
class LMVAE_Test():
    def __init__(self, lmvae, data, domain_mask, K_vec, samples=100):
        self.z = NewZ(lmvae, data, domain_mask, samples)      
