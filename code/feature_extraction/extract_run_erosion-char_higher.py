...
errors = pd.read_csv("data/errors_erosion-char_higher.csv")
...
# Begin the feature extraction loop:

N = logodata.shape[0]
errors = pd.Series()

for l in range(171,N):
    
    print("\r",str(l)+"/"+str(N),end='')
    ...
    try:
        ...
        if l==0:
            # Initalize the aggregated dfs:
            combined_colors = color_df
            combined_mark_features = mark_features
            combined_marks = marks
            combined_mark_hulls = mark_hulls
            combined_fonts = brand_font_df
            combined_summaries = brand_summary_df
            combined_hulls = brand_hull_df
        else:
    ...
    if l % 10 == 0:
        errors.to_csv("data/errors_erosion-char_higher.csv", index = False)
        combined_colors.to_csv("data/colors_erosion-char_higher.csv", index = False)
        combined_mark_features.to_csv("data/mark_features_erosion-char_higher.csv", index = False)
        combined_marks.to_csv("data/marks_erosion-char_higher.csv", index = False)
        combined_mark_hulls.to_csv("data/mark_hulls_erosion-char_higher.csv", index = False)
        combined_fonts.to_csv("data/fonts_erosion-char_higher.csv", index = False)
        combined_summaries.to_csv("data/summaries_erosion-char_higher.csv", index = False)
        combined_hulls.to_csv("data/hulls_erosion-char_higher.csv", index = False)
