...
nclust = np.max(features['scaled_cluster'])

for j in range(nclust+1):
    features_mat = features.loc[features['scaled_cluster']==j,'0':'624'].as_matrix()
    fracs = features.loc[features['scaled_cluster']==j,'frac'].as_matrix()
    ...
    scipy.misc.imsave('data/clustering/marks-scaled/'+str(j)+'.jpg', joined_examples)
...
cluster_labels.to_csv("data/clustering/marks-scaled_descs.csv")
